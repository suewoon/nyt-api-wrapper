...
class Scraper(object):
	...
	def scrape_and_save(self, params):
		...
		# hits shows literally how many pages are there 
		max_pages = int(response_json['response']['meta']['hits']/10) + 1

		metadata = self.query_metadata(response_json)
		page_contents_data = self.scrap_pages(response_json)

		df = pd.DataFrame(metadata, columns=self.columns[:-1])
		df[self.columns[-1]] =  page_contents_data
		
		records = json.loads(df.T.to_json()).values()
		db.articles.insertMany(records)
	...
	def query_metadata(self, response_json):
		...
		selected_column = self.columns[:-1]
		
		# extract the data
		metadata = []
		for row in response_json['response']['docs']:
			selected_row = []
	...
	def scrap_pages(self, response_json):
		'''
		:loop over response in order to get web_url of each object.
		@param response_json: api response in json format
		@return: a list of page contents string 
		'''
		n_contents = len(response_json['docs'])
		page_contents = []

		for i in range(n_contents):
			body_text_str = self.scrap_page(response_json['docs']['web_url'])
...
if __name__ == '__main__':
	...
	params = {'from_date':args.from_date, 'to_date':args.to_date, 'keyword':args.keyword}
